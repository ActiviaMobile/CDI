...
export const getTokenRequest = async (setUser, updateToken, passport) => {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4) {
            if (xmlhttp.status === 200) {
                ...
                console.log('token request' , rsp[0].value)
                sessionStorage.setItem('token', rsp[0].value);
                updateToken(rsp[0].value)
                wsRequest(setUser,rsp[0].value,'UsuarioGetAll')
            }
        }
    }
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4) {  
            if (xmlhttp.status === 200) {
                ...
                console.log('ws request' , res[0].value)
